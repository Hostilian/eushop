...
# Trigger: Repeat every hour starting in 2 minutes
...
try {
    Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $TriggerRepeat -Settings $Settings -User $env:USERNAME -Force | Out-Null
    Write-Host "[OK] Main daemon task registered: $TaskName" -ForegroundColor Green
} catch {
    Write-Warning "First registration attempt failed - retrying with basic parameters..."
    try {
        Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $TriggerRepeat -User $env:USERNAME -Force | Out-Null
        Write-Host "[OK] Main daemon task registered: $TaskName" -ForegroundColor Green
    } catch {
...
$WatchdogLines = @(
    ...
    '            Write-Host "WATCHDOG: Pool stale. Triggering refresh..."',
    '            & $Python $DaemonScript --once 2>&1',
    '        } else {',
    '            Write-Host "WATCHDOG: Pool OK"',
    '        }',
    '    } catch {',
    '        & $Python $DaemonScript --once 2>&1',
    '    }',
...
$WatchdogLines | Set-Content -Path $WatchdogScript -Encoding UTF8
...
try {
    Register-ScheduledTask -TaskName $WatchdogTaskName -Action $WatchdogAction -Trigger $TriggerWatchdog -Settings $WatchdogSettings -User $env:USERNAME -Force | Out-Null
    Write-Host "[OK] Watchdog task registered: $WatchdogTaskName (every 15 min)" -ForegroundColor Green
} catch {
    Write-Warning "Failed to register watchdog: $_"
}
