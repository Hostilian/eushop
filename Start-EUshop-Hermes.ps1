...
function Invoke-SelfRepair {
    ...
    # Repair 5: Reset crash counter if it's corrupt
    try {
        Get-CrashCounter | Out-Null
    } catch {
        Reset-CrashCounter
        Write-Log "Repaired: Reset corrupt crash counter." "SUCCESS" "repair"
        $repaired++
    }
