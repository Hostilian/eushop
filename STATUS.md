...
| **Backend** | API Gateway Routing | Phase 0 | **Implemented** | Express proxy/routing, request logger, error middleware, and Zod parser. | [api-gateway/src](file:///d:/CODING/eushop/services/api-gateway/src) |
...
| **Auth** | User Authentication | Phase 1 | **In Progress** | Current flow uses mock base64 token gateway and localStorage session. | [auth.ts (Gateway)](file:///d:/CODING/eushop/services/api-gateway/src/middleware/auth.ts), [services.ts (Web)](file:///d:/CODING/eushop/apps/web/lib/services.ts) |
...
| **Compliance** | KYBC & DAC7 Intake | Phase 2 | **In Progress** | Forms collect tax and business registration, backend saves them, frontend submit is pending integration. | [BecomeSellerRequest.java](file:///d:/CODING/eushop/services/core-service/src/main/java/com/eushop/core/dto/BecomeSellerRequest.java), [become-seller.tsx](file:///d:/CODING/eushop/apps/web/pages/become-seller.tsx) |
...
- Proxies requests to Core Service on port `3001`.

### 4. Local Development Lifecycle
- `pnpm` workspaces routing the Next.js frontend (`apps/web`) and Express API Gateway (`services/api-gateway`).
...
- Authentication uses a mock token exchange gateway (base64 payload mimicking JWTs) and persists user information in `localStorage` on the frontend.
- Gateway includes Auth0 signature checking structure but is bypassed when `USE_MOCK_AUTH=true`.
