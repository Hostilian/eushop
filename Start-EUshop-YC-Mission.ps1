...
$script:FccPid     = -1   # -1 = not started; >0 = running PID
$script:HermesPid  = -1
$script:Stop       = $false
...
function Start-FccLoop {
    ...
    if (-not (Get-Command "fcc-work" -ErrorAction SilentlyContinue)) {
        ...
        $script:FccPid = -1  # Sentinel: not running
        return
    }
    ...
    try {
        ...
        Write-Log "Start-Process failed for FCC loop: $($_.Exception.Message)" "ERROR"
        $script:FccPid = -1
        return
    }

    if ($null -eq $proc) {
        Write-Log "Start-Process returned null for FCC loop. FCC will not run." "ERROR"
        $script:FccPid = -1
        return
    }
...
function Watch-Agents {
    if ($script:FccPid -and $script:FccPid -gt 0) {
        Write-Log "Watching FCC (PID $script:FccPid). Press Ctrl+C to stop." "INFO"
    } else {
        Write-Log "FCC not running (no PID). Hermes job is the active agent. Press Ctrl+C to stop." "WARN"
    }
    try {
        while (-not $script:Stop) {
            if ($script:FccPid -gt 0 -and -not (Test-ProcessAlive -Id $script:FccPid)) {
        ...
        if ($script:FccPid -gt 0 -and (Test-ProcessAlive -Id $script:FccPid)) {
...
# Register Ctrl+C handler (suppressed in non-interactive contexts)
    ...
    { $_ -in "Both", "FixAuth" } {
        ...
        $hermesJob = Start-Job -ScriptBlock {
            ...
            while ($true) {
                $failures++
                try {
                    # Run with -Provider auto so Hermes picks the best available
                    # provider (FCC first, then openrouter, then others).
                    # Do NOT pass -NoRestart: let Start-EUshop-Hermes handle its
                    # own restart loop; outer try/catch catches Python crashes.
                    & powershell.exe -NoProfile -ExecutionPolicy Bypass `
                        -File $Launcher `
                        -ProjectPath $Proj `
                        -Provider auto `
